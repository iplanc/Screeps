 ...
 * @LastEditTime: 2020-09-16 18:00:05
 * @FilePath: \Screeps\role.harvester.js
 */
...
var roleHarvester = {
	...
	run: function(creep) {
		if (creep.store.getFreeCapacity() > 0) {
			...
			if (creep.room.name == 'W34N2') {
				var link = Game.getObjectById('5e5374800a99ae4be2286c7a');
				if (link) {
			...
			switch (creep.room.controller.level) {
				...
				case 3:
					var targets = creep.room.find(FIND_STRUCTURES, {
						filter: (i) =>  (i.structureType == STRUCTURE_TOWER   && i.store[RESOURCE_ENERGY] < 1000) ||
